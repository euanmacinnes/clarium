...
- Start the server (HTTP on 7878 by default):
  - cargo run --release --bin timeline_server
- Enable the pgwire endpoint (on 5433) by building with the feature and turning it on:
  - cargo run --release --features pgwire --bin timeline_server -- --pgwire
- Configure ports and the database root via flags or environment (flags win):
  ...
  - Env: TIMELINE_HTTP_PORT, TIMELINE_PG_PORT, TIMELINE_DB_FOLDER, TIMELINE_PGWIRE
  - Examples (PowerShell):
...
Chunking strategy and mid‑timeline inserts
...
- Inserts in the middle of the timeline (backfills)
  - Backfilling earlier data is supported. A backfill simply creates a new chunk whose <min_time>…<max_time> falls inside (or overlaps) the existing timeline.
...
- Endpoints:
  ...
  - POST /query
    - Body: {"query": "SELECT AVG(temp), _time FROM timeline/public/demo.time BY 1m"}
    - Returns JSON with status and rows.
  - POST /use/database {"name":"timeline"}
...
- SELECT <expr_list> FROM <db/schema/table[.time]> [BY <window> | GROUP BY <cols> | ROLLING BY <window>] [WHERE <expr>] [HAVING <expr>] [ORDER BY <col> [ASC|DESC], ...] [LIMIT <n>]
  ...
  - ORDER BY: optional, supports multiple columns with ASC/DESC. Columns must exist in the projection (aliases allowed). For time-series tables, ORDER BY is not required for LIMIT to return the first/last rows in natural time order.
    - BY SLICE: The BY clause can accept a full SLICE plan to define custom aggregation windows. Example: SELECT AVG(v) FROM timeline/public/demo.time BY SLICE( USING (2025-01-01T00:00:00Z, 2025-01-01T01:00:00Z) UNION (2025-01-01T03:00:00Z, 2025-01-01T04:00:00Z) ). Disallows mixing with GROUP BY; string functions in SELECT are not supported with BY SLICE. When labels are produced by the SLICE plan (via USING LABELS or auto-derived), those label columns are automatically surfaced in the SELECT result alongside _time and aggregates.
  ...
  - Examples:
    - SELECT _time, v FROM timeline/public/demo.time WHERE _time BETWEEN 2025-01-01T00:00:00Z AND 2025-01-01T01:00:00Z LIMIT 10
    - SELECT * FROM timeline/public/demo.time WHERE _time BETWEEN 0 AND 1000
    - SELECT id, name FROM regular/ns/users ORDER BY name DESC LIMIT 5
    - SELECT _time, v FROM timeline/public/demo.time LIMIT -100  -- last 100 rows
...
- Examples:
  - SELECT _time, a, b FROM timeline/public/sensors.time WHERE a > 0 INTO timeline/public/output.time
  - SELECT a, b FROM timeline/public/regular_src INTO timeline/public/regular_dest
  - SELECT a, b FROM timeline/public/regular_src INTO timeline/public/regular_dest REPLACE
- Notes:
...
- Authentication is required (default admin is timeline/timeline). Use sslmode=disable for local testing.
- Current database and schema default to timeline/public. You can run CREATE TABLE and INSERT; SELECT streams results. All columns are returned as text for simplicity.
...
- Examples (psql):
  - psql "host=127.0.0.1 port=5433 dbname=timeline user=timeline sslmode=disable"
  - CREATE TABLE timeline.public.metrics (value float, label text);
  - INSERT INTO timeline.public.metrics (_time, value, label) VALUES (extract(epoch from now())*1000, 1.23, 'a');
  - SELECT COUNT(*) FROM timeline.public.metrics;
...
- Environment configuration (also used by clarama.deploy.yaml):
  - TIMELINE_HTTP_PORT (default 7878)
  - TIMELINE_PG_PORT (default 5433)
  - TIMELINE_PGWIRE (true/false; only effective if built with --features pgwire)
  - TIMELINE_DB_FOLDER (default dbs when running locally; in k8s typically /etc/clarama/data/{instance}/timeline)
- Kubernetes deployment integration:
  - clarama.deploy.yaml defines templated variables for Timeline and injects the above environment variables into the Timeline pod.
  - Ensure a persistent volume is mounted at /etc/clarama/data so the DB folder exists and is writable. Example path: /etc/clarama/data/{clarama_instance}/timeline.
- Startup diagnostics for permissions:
  - On startup the server logs the folder configuration (cwd, exe, user, home, PWD, db_root, TIMELINE_DB_FOLDER) and whether key paths exist.
...
- The timeline_cli binary can execute queries locally or act as an interpreter for a remote server.
- One‑shot local queries:
  - cargo run --bin timeline_cli -- --query "SELECT COUNT(_time) FROM timeline/public/demo.time" [--root dbs]
  - echo "SCHEMA SHOW timeline/public/demo.time" | cargo run --bin timeline_cli --
- REPL mode and remote connect:
  - cargo run --bin timeline_cli -- --repl
...
- On empty stores a demo dataset is created at timeline/public/demo.time (sine wave, 7 days, 1s cadence).
...
- Examples:
  - SLICE USING timeline/public/maintenance.time ON start_ms end_ms
...
- The Timeline server uses the `tracing` crate with `tracing_subscriber::EnvFilter` initialized in `src/main.rs`. Log verbosity is controlled via the `RUST_LOG` environment variable.
...
- Development (verbose on core modules):
  - `timeline=debug,timeline::pgwire=debug,timeline::exec=debug,timeline::system=debug,timeline::parser=debug,info`
- Production (concise):
  - `info` or `timeline=info`

Enable logging (Windows PowerShell)

- One‑shot in current shell, then run the server with cargo:
  - `$env:RUST_LOG = "timeline=debug,timeline::pgwire=debug,timeline::exec=debug,timeline::system=debug,timeline::parser=debug,info"`
  - `cargo run --release --bin timeline_server`
...
- One‑shot in current shell, then run the server with cargo:
  - `export RUST_LOG="timeline=debug,timeline::pgwire=debug,timeline::exec=debug,timeline::system=debug,timeline::parser=debug,info"`
  - `cargo run --release --bin timeline_server`
- With pgwire:
  - `export RUST_LOG="timeline=debug,timeline::pgwire=debug,timeline::exec=debug,timeline::system=debug,timeline::parser=debug,info"`
  - `cargo run --release --features pgwire --bin timeline_server -- --pgwire`
...
- Use module filters to reduce noise while keeping the area of interest at `debug`:
  - Example: `timeline::system=trace,timeline::pgwire=debug,info`
- Keep production at `info` or higher to avoid verbose output and performance overhead.
- Logs can be redirected to a file using your shell (e.g., `... 2>&1 | tee timeline.log`).
