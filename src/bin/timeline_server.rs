//!
//! Timeline server binary
//! ----------------------
//! Command-line entry point for starting the Timeline HTTP server and optional
...
async fn main() -> Result<()> {
    println!(r"___________.__               .__  .__
\__    ___/|__| _____   ____ |  | |__| ____   ____
  ...
  |____|   |__|__|_|  /\___  >____/__|___|  /\___  >
    ...
    if has_flag(&args, "--help") || has_flag(&args, "-h") {
        println!("Timeline Server\n\nUSAGE:\n  timeline_server [--http-port N] [--pg-port N] [--db-folder PATH] [--pgwire|--no-pgwire]\n\nOPTIONS:\n  --http-port N       HTTP API port (env: TIMELINE_HTTP_PORT, default 7878)\n  --pg-port N         pgwire port (env: TIMELINE_PG_PORT, default 5433)\n  --db-folder PATH    Database root folder (env: TIMELINE_DB_FOLDER, default dbs/timeline)\n  --pgwire [bool]     Enable pgwire (env: TIMELINE_PGWIRE). Presence enables; or pass true/false.\n  --no-pgwire        Disable pgwire explicitly.\n");
        return Ok(());
    }
    ...
    let env_http = parse_port_env("TIMELINE_HTTP_PORT");
    let env_pg = parse_port_env("TIMELINE_PG_PORT");
    let env_root = std::env::var("TIMELINE_DB_FOLDER").ok();
    let env_pgwire = parse_bool_env("TIMELINE_PGWIRE");
    ...
    #[cfg(feature = "pgwire")]
    {
        ...
        if enable_pgwire {
            println!(
                "Timeline starting using ports: http={}, pgwire={}, db_root={}",
                http_port, pg_port, db_root
            );
            tracing::info!("Using ports: http={}, pgwire={}, db_root={}", http_port, pg_port, db_root);
        } else {
            println!(
                "Timeline starting with pgwire DISABLED: http={}, db_root={}",
                http_port, db_root
            );
            tracing::info!("pgwire disabled; Using port: http={}, db_root={}", http_port, db_root);
        }
        return timeline::server::run_with_ports(http_port, pg_opt, &db_root).await;
    }
    ...
    #[cfg(not(feature = "pgwire"))]
    {
        ...
        println!("Timeline starting using ports: http={}, db_root={}", http_port, db_root);
        tracing::info!("Using port: http={}, db_root={}", http_port, db_root);
        // Pass None for pgwire (disabled)
        return timeline::server::run_with_ports(http_port, None, &db_root).await;
    }
}
