//! Intermittent failure harness
//! Runs selected UDF tests multiple times in a single #[test] to try to catch
...
/// Reset thread-local execution flags to their defaults.
/// Note: we no longer reset the global registry between sub-tests; UDFs are loaded once at harness start.
fn reset_test_environment() {
    // Reset system flags to their defaults
    system::set_null_on_error(true);
    system::set_strict_projection(true);
}

fn run_scalar_udf_where_concat_arith_and_nulls() {
...
fn run_scalar_multi_return_projection_and_misuse() {
    let prev = system::get_strict_projection();
    system::set_strict_projection(false);
    let (shared, db) = setup_basic_table();
...
fn run_aggregate_udf_group_by_single_and_multi() {
    let prev = system::get_strict_projection();
    system::set_strict_projection(false);
...
fn run_aggregate_udf_error_policy() {
...
#[test]
fn intermittent_failure_harness() {
    // Load all UDFs once at the start so all sub-tests can use them
    super::udf_common::init_all_test_udfs();
    
    // Number of iterations to try; intermittent failures have been seen within 4-5 iterations
    let iterations = 8usize;
    let mut failures: Vec<(usize, String, String)> = Vec::new();

    // small helper to run and catch panics, printing Lua/system diagnostics
    let mut run_with_catch = |iter_idx: usize, name: &str, f: fn()| {
        let t0 = Instant::now();
        tprintln!("[intermittent] running: {} (iter {} of {})", name, iter_idx + 1, iterations);
        // Ensure a clean environment before each run
        ...
        match res {
            Ok(_) => {
                tprintln!("[intermittent] ok: {} in {} ms", name, dt.as_millis());
                // Also reset after success to avoid carry-over to the next sub-test
                reset_test_environment();
            }
            Err(payload) => {
                ...
                // Capture backtrace and global registry snapshot
                let bt = std::backtrace::Backtrace::force_capture();
                let reg = crate::scripts::debug_script_registry_snapshot().unwrap_or_else(|| "<no-registry>".to_string());
                let strict = system::get_strict_projection();
                let null_on_err = system::get_null_on_error();
                tprintln!("[intermittent][PANIC] {} at iter {}: {}", name, iter_idx + 1, msg);
                tprintln!("[intermittent][PANIC] system: strict_projection={} null_on_error={}", strict, null_on_err);
                tprintln!("[intermittent][PANIC] registry:\n{}", reg);
                tprintln!("[intermittent][PANIC] backtrace:\n{}", bt);
                failures.push((iter_idx, name.to_string(), msg));
                // Reset after failure to allow subsequent sub-tests to run in a clean environment
                reset_test_environment();
            }
        }
    };
    ...
    for i in 0..iterations {
        tprintln!("[intermittent] iteration {} / {}", i + 1, iterations);
        run_with_catch(i, "test_aggregate_udf_error_policy", run_aggregate_udf_error_policy);
        run_with_catch(i, "test_aggregate_udf_group_by_single_and_multi", run_aggregate_udf_group_by_single_and_multi);
        run_with_catch(i, "test_scalar_multi_return_projection_and_misuse", run_scalar_multi_return_projection_and_misuse);
        run_with_catch(i, "test_scalar_udf_where_concat_arith_and_nulls", run_scalar_udf_where_concat_arith_and_nulls);
    }

    if failures.is_empty() {
        tprintln!("[intermittent] completed all iterations without failure");
    } else {
        tprintln!("[intermittent] observed {} failure(s):", failures.len());
        for (iter_idx, name, msg) in &failures {
            tprintln!("  - iter {}: {} => {}", iter_idx + 1, name, msg);
        }
        // Fail at the very end to keep cargo test exit semantics, after running all iterations
        panic!("intermittent harness observed {} failure(s)", failures.len());
    }
}
