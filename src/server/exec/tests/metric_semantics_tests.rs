...
fn metric_semantics_and_tiebreaks() {
    ...
    let q1_parsed = match query::parse(q1).unwrap(){ Command::Select(s)=>s, _=>unreachable!() };
    let df1 = run_select(&store, &q1_parsed).unwrap();
    ...
    let q2_parsed = match query::parse(q2).unwrap(){ Command::Select(s)=>s, _=>unreachable!() };
    let df2 = run_select(&store, &q2_parsed).unwrap();
    ...
    let q3_parsed = match query::parse(q3).unwrap(){ Command::Select(s)=>s, _=>unreachable!() };
    let df3 = run_select(&store, &q3_parsed).unwrap();
